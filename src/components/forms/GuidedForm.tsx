...
import { inquiryConsent } from "@/content/inquiry";
import { validateAgainst, stepIndexOfField, type FieldErrors } from "@/lib/validation/inquiry";
...
/** The strings a stepped form needs. Both `inquiryCopy` and `enrichmentCopy` satisfy this. */
export interface GuidedFormCopy {
  submitLabel: string;
  submittingLabel: string;
  continueLabel: string;
  backLabel: string;
  errorSummaryHeading: string;
  stepIndexHeading: string;
...
interface GuidedFormProps {
  ...
  copy: GuidedFormCopy;
  action: (previous: InquiryState, formData: FormData) => Promise<InquiryState>;
  initialState: InquiryState;
  /**
   * Consent is a condition of the inquiry itself. The pre-call enrichment form follows an
   * inquiry that already gave it, and asking twice would imply the first one lapsed.
   */
  withConsent?: boolean;
  /**
   * Whether this form's steps belong to the measured funnel. Only the inquiry does — firing
   * `inquiry_step_reached` from enrichment as well would mix two different populations into
   * one series and quietly make the drop-off numbers wrong.
   */
  track?: boolean;
}

/**
 * A stepped form: the guided inquiry and the pre-call enrichment form are both this component.
 *
 ...
 *
 * That degradation has one consequence worth knowing: anything reading the page without running
 * scripts sees every field at once. When the inquiry was 25 fields in six groups, that is what a
 * crawler saw, and it is the likeliest reason the form read as far heavier than it was.
 *
 ...
 * — the scale the hero uses — and the page's own H1 steps down to make room. Deliberately NOT a
 * Typeform-style one-field-per-screen rebuild: more screens is more clicking, not better.
 *
...
export function GuidedForm({
  ...
  useEffect(() => {
    ...
    // compare against. Firing it makes every step one self-contained series.
    if (track) {
      trackEvent(analyticsEvents.inquiryStepReached, inquiryStepPayload(0, steps[0]?.name ?? ""));
    }
  }, [track, steps]);
  ...
  useEffect(() => {
    ...
    setStep(Math.min(...names.map((name) => stepIndexOfField(name, steps))));
  ...
  useEffect(() => {
    if (!track || state.status === "idle") return;
  ...
  useEffect(() => {
    if (track && pending) {
      trackEvent(analyticsEvents.inquirySubmitted, inquirySubmittedPayload(step + 1));
    }
  }, [track, pending, step]);

  const errors: FieldErrors = { ...state.fieldErrors, ...clientErrors };
  const currentStepErrorNames = Object.keys(errors).filter(
    (name) => stepIndexOfField(name, steps) === step
  );
  ...
  function validateStep(index: number): boolean {
    const { fieldErrors } = validateAgainst(steps, readValues(), { requireConsent: withConsent });
    const stepFieldNames = new Set(steps[index]?.fields.map((f) => f.name) ?? []);
    if (withConsent && index === lastStep) stepFieldNames.add(inquiryConsent.name);
  ...
  function goTo(index: number) {
    ...
    if (track) {
      trackEvent(
        ...
        inquiryStepPayload(index, steps[index]?.name ?? "")
      );
    }
  ...
  // 0..1, consumed by the progress device in globals.css. The first step sits one notch along
  // rather than at zero: "one of three" reads as begun, and an empty track reads as broken.
  ...
  return (
    <form
      ...
      // otherwise animate every section at once on load.
      ...
      <div className="lg:grid lg:grid-cols-[minmax(0,38rem)_minmax(11rem,1fr)] lg:gap-16">
        <div>
          ...
          {showSummary ? (
            ...
            >
              <h2 className="text-h3 font-display">{copy.errorSummaryHeading}</h2>
          ...
          <div className="mt-12 flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            {/* Distinct keys, and they matter. Both branches render an <ActionButton>, so
                ...
                verified — the exact element that said "Continue" (type=button) becomes the
                submit button. A visitor who clicks Continue and clicks again in the same spot
                a moment later therefore submits, rather than doing nothing. Separate keys make
                React swap the node instead of mutating it. */}
            {enhanced && step < lastStep ? (
              ...
              <ActionButton
                ...
                variant="primary"
                busy={pending}
                busyLabel={copy.submittingLabel}
              >
                {copy.submitLabel}
              </ActionButton>
            )}
        ...
        {/* The whole shape of the form, standing. Enhanced only: without JavaScript every step
            is already on screen, so an index marking a "current" one would describe something
            that is not happening. An <aside>, never a <section> — the no-JS guard counts
            `form section` and expects exactly the steps.
            ...
            <ol className="mt-4">
              {steps.map((s, index) => (
                ...
                >
                  {s.name}
                  {index === step ? (
                    <span className="sr-only">{copy.currentStepSuffix}</span>
                  ) : null}
                </li>
              ))}
            </ol>
